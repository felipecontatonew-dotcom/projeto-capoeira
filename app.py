# --- PASSO CRÍTICO: MONKEY PATCHING ---
# Estas duas linhas DEVEM ser as primeiras de todo o código
import eventlet
...
def cadastro_aluno():
    if request.method == 'POST':
        ...
        new_user = User(matricula=matricula_auto, nome=form['nomeAluno'], idade=int(form['idadeAluno']), graduacao=form['graduacaoAluno'], email=form['emailAluno'], data_entrada=datetime.strptime(form['dataEntrada'], '%Y-%m-%d').date(), role='admin' if form.get('tipo_usuario') == 'adm' else 'aluno')
